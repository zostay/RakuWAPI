...
[B<Conjecture:> The above code does not work, but should. The problem is that C<.assuming()> does not (as of this writing) preserve the return type of the method called. I have suggested a patch to correct this problem. If that patch or one like it turns out to somehow go against the spirit of C<.assuming()> or something, this requires a better example.]
