...
The environment MUST be an L<Associative>. The keys of this map are mostly derived from the old Common Gateway Interface (CGI) as well as a number of additional P6SGI-specific values. The application server MUST provide each key as the named type. All variables given in the table below MUST be supported, except for those with the C<p6wx.> prefix.
...
=begin table :allow<C>
    ...
    C<p6w.protocol>          | C<< Str:D >>                           | This is a string naming the
                             ...
                             |                                        | call. This specification defines
                             |                                        | "http", "ws", and "socket" protocols.
...
The application server supplies a L<Set> of strings to the application in C<p6w.protocol> that specifies the way in which the application server expects the application to response. This specification defines the following response protocols: "http", "ws", and "socket". Application servers SHOULD support both of these protocols when appropriate for the C<SERVER_PROTOCOL>.
...
The application MUST return a valid P6SGI response to the server. The format required for this response is determined by examining the C<p6w.protocol> variable in the environment. This specification defines the response required when this variable is set to "http" or "ws" or "socket".
...
When C<p6w.protocol> is "http" and the C<SERVER_PROTOCOL> is "HTTP/2", servers SHOULD support the HTTP/2 push promises extension. However, applications SHOULD check to make sure that the C<p6wx.h2.push-promise> variable is set before using this extension.
...
These protocols may have different interfaces. We want to provide a means by which servers and applications may implement these alternate protocols, which each may have different requirements. To facilitate this, P6SGI provides the C<p6w.protocol> variable in the environment. The protocol defined here tells the application what the server is providing and how the application is expected to respond.

This specification defines the following protocols:

=item B<http> for HTTP-style request-response protocols

=item B<ws> for WebSocket-style generic socket protocols

=item B<socket> for raw, plain socket protocols

=head2 4.0 HTTP Protocol
...
An application MUST return a L<Promise>. This Promise MUST be kept with a L<Capture> (or something that becomes one on return). It MUST contain 3 positional elements, which are the status code, the list of ehaders, and the message payload.
...
=item The C<p6w.protocol> MUST be set to "ws".
...
=item L<Blob>. Any Blob emitted by the application SHOULD be treated as binary data, framed exacctly as is, and returned to the client.
...
=item If the application object provides a C<provide-app> method, the application server MUST call that method prior to every call to the application routine. It will pass a single argument: the application environment to the method. The return value will be used as the application routine to use for only that request. The application MUST return a Callable to be used to handle that request to the application.
...
=item Adding C<p6w.protocol> to handle server-to-application notification of the required response protocol.
...
=item Added Section 4 to describe protocol-specific features of the specification. Section 4.0 is for HTTP, 4.1 is for WebSocket, and 4.2 is for Socket.
