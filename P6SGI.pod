...
The C<p6wx.header.done> environment variable, if provided, MUST be a vowed L<Promise>. This Promise MUST be kept when the server is done sending or processing the response header. The Promise MUST be broken if the server is unable to or will not send the application provided headers.
...
The C<p6wx.body.done> environment variable, if provided, MUST be a vowed L<Promise>. This Promise MUST be kept when the server is done sending or processing the response body. The Promise MUST be broken if the server is unable to or will not send the complete application body.
...
The C<p6wx.cleanup.handlers> environment variable MUST be provided if the C<p6wx.cleanup> flag is set. This MUST an L<Array>. The application adds cleanup handlers to the list by putting L<Callable>s into the Array (usually by C<push>ing). Each handler will be given a copy of the C<%env> as the first argument.
...
The C<p6wx.protocol.upgrade> environment variable MUST be provided if the server implements the protocol upgrade extension. It MUST be set to an L<Array> of protocol names of protocols the server supports.
...
Servers that support this protocol upgrade MUST place the name "h2c" and/or "h2" into the C<p6wx.protocol.upgrade> array, for support of HTTP/2 over cleartext connections and HTTP/2 over TLS, respectively.
...
Servers that support the WebSocket protocol upgrade MUST place the name "websocket" into the C<p6wx.protocol.upgrade> array.
