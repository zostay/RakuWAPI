...
=begin table :allow<C>
    ...
    C<p6sgix.header.done>    | C<< Promise:D >>                       | A vowed Promise that is kept by the
                             |                                        | server when the server is done
                             |                                        | sending or processing the header. It
                             |                                        | will be broken if the server is unable
                             |                                        | to send the application header.

    C<p6sgix.body.done>      | C<< Promise:D >>                       | A vowed Promise that is kept by the
                             |                                        | server when the server has finished
                             |                                        | sending or processing the response.
                             |                                        | It will be broken if the server
                             |                                        | terminates processing early.
