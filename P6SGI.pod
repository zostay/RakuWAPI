...
=item The message body is typically returned as a L<Supply> that emits zero or more L<Str> and L<Blob> objects that are encoded, if necessary, and concatenated together to form the finished message body.
...
Unless the status code is one that is not permitted to have a message body, the application server MUST tap the Supply and process each emitted L<Blob> or L<Str>, until the the either the Supply is done or the server decides to quit tapping the stream for some reason.
...
=item The message body MUST be a L<Supply> that emits L<Str> and L<Blob> objects or an object that coerces into such a Supply (e.g., a List or an Array).
