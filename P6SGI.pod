...
=begin table :allow<C>
    ...
    C<p6w.protocol>          | C<< Set:D >>                           | This is a L<Set> containing the names
...
The application MUST return a valid P6SGI response to the server. The format required for this response is determined by examining the C<p6w.protocol> variable in the environment. This specification defines the response required when this variable is set to "http" or "ws".
...
=head3 4.0.1 Status Code

=head3 4.0.2 Response Headers

=head3 4.0.3 Message Payload
...
=item The server MUST decode frames received from the client and emit each of them to C<p6wx.input>. The frames MUST NOT be buffered or concatenated.

=item The server's supplied C<p6wx.input> L<Supply> must be I<sane>. The server SHOULD signal C<done> through the Supply when the client closes the WebSocket connection normally and C<quit> on abnormal termination of the connection.
...
=item Added Section 4 to describe protocol-specific features of the specification.
