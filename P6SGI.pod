...
This specification suggests adopting a file name suffix of F<.p6w> (Perl 6 Web application files) to identify these files when an extension is appropriate.
...
=item C<psgi.> is for PSGI legacy standard environment.

=item C<psgix.> is for PSGI legacy extensions to the environment.

=head3 2.0.2 The Input Stream

The input stream is set in the C<p6sgi.input> key of the environment. The server MUST provide an object that implements a subset of the methods of L<IO::Handle>. It MAY provide an L<IO::Handle>.
...
=item B<Wrapper Component>. The wrapper component is the only optional part. It describes how the container component is delivered with the remaining parts. For example, an application may return a L<Promise> that is kept with the container component or it may be omitted and the container returned directly by the application.

=item B<Container Component>. The container component may be returned directly or may be returned within a wrapper component. It supplies the remaining three parts: status code, headers, and body. For example, a 3-element L<Positional> is the container component with the elements referring to the status code, headers, and body, respectively.

=item B<Status Code Component>. The status code component declares the HTTP status code the application is returning. For example, the status code is returned as an integer for status codes, like 200.

=item B<Header Component>. The header component defines the headers the application intens to return to the client. For example, the header component is typically returned as a L<List> of L<Pair>s, with each key naming a header and each value being the value to set for that header.

=item B<Body Component>. The body component defines the content of the message body. For example, the body component may be returned as a L<Supply> that emits a series of L<Str>s and L<Blob>s.

These components may vary independent of one another. For example, the body component might be returned as L<Supply> or as an iterable list of L<Str>s and L<Blob>s instead and the body may be returned by a container directly or a container kept by a L<Promise>. Furthermore, framework and application developers may wish to experiment with variations not described here. This is permitted, but such variations SHOULD be easy to distinguish from the standard forms and SHOULD also be easy to distinguish from each other. Such extensions can be universally supported by providing middleware to convert these response forms into the standard forms supported by all P6SGI servers and middleware.

=head4 2.0.4.0 Wrapper Components
...
Only a single container component form is defined here. The container is given as a L<Positional> with exactly three elements.
...
=head5 2.0.4.3.0 List of Pairs

The headers are returned as a L<List> of L<Pair>s, each Pair maps a header name to a header value.
...
The server SHOULD examine the L<Content-Type> header for the C<charset> setting. This SHOULD be used to aid in encoding any L<Str> encountered when processing the Message Body. If the application does not provide a C<charset>, the server MAY choose to add this header itself using the encoding provided in C<p6sgi.encoding> in the environment.

The server SHOULD examine the L<Content-Length> header, if given. It MAY choose to stop consuming the Message Body once the number of bytes given has been read.

The test for this component is:

    $header ~~ List && $header.all ~~ Pair

=head4 2.0.4.4 Message Body

The message body may be given in one of the following two forms.

=head5 2.0.4.4.0 Supply of Str and Blob

The best and most flexible form, but not necessarily the most straightforward, is to use a L<Supply>. This L<Supply> will emit L<Blob> and L<Str> objects.

The server SHOULD NOT concern itself with whether the L<Supply> given is on demand or live. The application server MUST tap the Supply and process each emitted L<Blob> or L<Str>, until the the either the Supply is done or the server decides to quit tapping the stream for some reason.

The application server SHOULD continue processing emitted values until the Supply is done or until L<Content-Length> bytes have been emitted. The server MAY stop tapping the Supply for various other reasons as well, such as timeouts or because the client has closed the socket, etc.
...
=head5 2.0.4.4.1 Iterable of Str and Blob

This variation may be treated the same as the L<Supply> form by using C<from-list> to generate a Supply from this List. All the specifications for such bodies apply here.
...
As with an application, middleware MUST return a valid P6SGI response to the server. Middleware developers SHOULD write design their middleware to return the best, most flexible form available. This is the form that returns a L<Promise> wrapper around a L<Positional> container containing an L<Int> status, L<List> of L<Pair>s header, and a L<Supply> of L<Str> and L<Blob> message body.
...
In addition, the middleware application SHOULD pass through the message body unchanged when it does not need to work with it. If the middlware application does need to work with the message body, it SHOULD NOT modify the encoding of the body unless necessary. Whenever possible, it SHOULD maintain L<Str> as L<Str> and L<Blob> as L<Blob>.
...
The application server is required to provide a L<p6sgi.errors> key with an L<IO::Handle>-like object capable of logging application errors. The application MAY choose to log errors here (or it MAY choose to log them wherever else it likes).
...
The best response form is a L<Promise> that is kept with a 3-element L<Positional> with the following indexes, respectively:

=item B<Status Code>. This is a standard HTTP status code.

=item B<Message Headers>. This is a L<List> of L<Pair>s naming the headers the application wishes to put into the response.
...
=head4 2.2.4.1 Message Body as a List

A second variation that an application MAY provide is a message body as a L<List> of L<Str> and L<Blob> rather than as a L<Supply>. This is slightly shorter than using a L<Supply> and is provided for convenience. Actually, any L<Iterable> object is supported.
 ...
 * Use C<p6sgi.> prefixes in the environment rather than C<psgi.>
