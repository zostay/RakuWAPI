...
The application server is required to provide a C<p6sgi.errors> variable in the environment with an L<IO::Handle>-like object capable of logging application errors. The application MAY choose to log errors here (or it MAY choose to log them wherever else it likes).
...
=head2 3.0 Environment Extensions

These are extensions to the standard environment.

=head3 3.0.0 Header Done
...
=head3 3.0.2 Raw Socket

The C<p6sgix.io> environment variable, if provided, MUST be the bare metal L<IO::Handle> used to communicate to the client.
...
=head3 3.0.6 Harikiri Flag

The C<p6sgix.harikiri> environment variable, if provided, MUST be a L<Bool>. If set to C<True> it signals to the application that the server supports harikiri mode, which allows the application to ask the server to terminate the current work when the request is complete. See 3.0.7.

=head3 3.0.7 Harikiri Commit Flag
...
=head3 3.0.8 Cleanup Handlers Flag

The C<p6sgix.cleanup> environment variable, if provided, MUST be a L<Bool>. If set to C<True> it tells the application that the server supports running cleanup handlers after the request is complete. See 3.0.9.

=head3 3.0.9 Cleanup Handlers Stack

The C<p6sgix.cleanup.handlers> environment variable MUST be provided if the C<p6sgix.cleanup> flag is set (see 3.0.8). This MUST an L<Array>. The application adds cleanup handlers to the list by putting L<Callable>s into the Array (usually by C<push>ing). Each handler will be given a copy of the C<%env> as the first argument.

If the server supports harikiri mode, it SHOULD allow the cleanup handlers to invoke harikiri mode if they set C<p6sgix.hariki.commit> (see 3.0.7).
