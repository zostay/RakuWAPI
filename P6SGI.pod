...
A D<P6SGI application> is a Perl 6 subroutine that expects to receive an environment form a I<application server> and returns a response each time it is called to be processed by that server.

A D<Web Server> is an application that processes requests and responses according to the HTTP protocol.

An D<application server> is a program that is able to provide an environment to a I<P6SGI application> and process the value returned from such such an application.
